...
# GLMM 3 ensemble model
ens_model_5 <- function (data, df, initial_test, stepsize, gr = NA, ...) {
  ...
  if (!is.na(gr)) {
    ...
    formula <- observed ~ 1 + f(id_area_1, m1, model = 'bym2', graph = gr, scale.model = TRUE, constr = FALSE) +
                              f(id_area_2, m2, model = 'bym2', graph = gr, scale.model = TRUE, constr = FALSE) +
                              f(id_area_3, m3, model = 'bym2', graph = gr, scale.model = TRUE, constr = FALSE)
